...
At the DeepEP CUDA level, the normal non-cached dispatch has one layout launch,
one `intranode::notify_dispatch` launch (`deep_ep.cpp:482-507`,
`intranode.cu:12-159`), and one intranode dispatch launch
(`deep_ep.cpp:304-322`, `:576-603`) plus CPU counter coordination; combine has
one queue/barrier reset and one intranode combine launch (`deep_ep.cpp:719-771`).
These are whole-tensor operations.  The stock event
