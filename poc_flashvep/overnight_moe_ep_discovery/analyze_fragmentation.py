...
def main() -> None:
    ...
    metrics = raw.groupby(["case_id", "active_experts_per_rank"], as_index=False).agg(
    ...
    ax.plot(metrics["active_experts_per_rank"], metrics["expert_ms"], marker="o", label="Expert")
    ax.plot(metrics["active_experts_per_rank"], metrics["critical_wall_ms"], marker="s", label="D/E/C wall")
    ax.set(xlabel="Active experts per rank (same rank load)", ylabel="Critical ms", title="H1: within-rank fragmentation")
    ax.set_xticks(metrics["active_experts_per_rank"]); ax.legend(); fig.tight_layout()
    fig.savefig(args.result / "fragmentation_latency.png", dpi=150); plt.close(fig)
    base_a1 = float(metrics.loc[metrics.active_experts_per_rank == 1, "expert_ms"].iloc[0])
    base_a2 = float(metrics.loc[metrics.active_experts_per_rank == 2, "expert_ms"].iloc[0])
    a16 = float(metrics.loc[metrics.active_experts_per_rank == 16, "expert_ms"].iloc[0])
    ...
    effect = (a16 - base_a2) / base_a2 * 100.0 if base_a2 else 0.0
    effect_a1 = (a16 - base_a1) / base_a1 * 100.0 if base_a1 else 0.0
    summary = {
        ...
        "critical_wall_effect_pct_A2_to_A16": (
            (float(metrics.loc[metrics.active_experts_per_rank == 16, "critical_wall_ms"].iloc[0]) - float(metrics.loc[metrics.active_experts_per_rank == 2, "critical_wall_ms"].iloc[0]))
            / float(metrics.loc[metrics.active_experts_per_rank == 2, "critical_wall_ms"].iloc[0]) * 100.0
        ),
