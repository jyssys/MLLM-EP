...
def aggregate(args: argparse.Namespace) -> None:
    ...
    summary = {"DEEPEP_TRAFFIC_MATRIX_SHAPE": gate, "family_a_status_by_scale": status_by_scale, "family_a_effects": effects, "family_b_hotspot_full_path": hotspot, "stage_c_real_trace": "NOT_RUN_ARTIFACT_LIMITATION", "stage_c_reason": "Reused route artifacts have expert IDs but no source-rank attribution or live dispatch/combine timestamps.", "num_rank_files": len(rank_files), "num_raw_samples": len(all_samples), "physical_gpus": PHYSICAL_GPUS, "torch": json.loads((rank_files[0]).read_text(encoding="utf-8"))["torch"] if rank_files else None, "cuda": json.loads((rank_files[0]).read_text(encoding="utf-8"))["cuda"] if rank_files else None, "policy": POLICY}
