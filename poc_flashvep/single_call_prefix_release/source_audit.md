...
The high-throughput intranode path (the normal non-cached call used here)
contains:

* one `get_dispatch_layout` launch (`deep_ep.cpp:304-322`) and one
  `intranode::notify_dispatch` launch (`deep_ep.cpp:482-507`, kernel wrapper
  `intranode.cu:12-159`), followed by one `intranode::dispatch` launch
  (`deep_ep.cpp:576-603`), plus CPU counter coordination
  (`deep_ep.cpp:517-537`); and
