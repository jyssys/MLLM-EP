...
"$VENV/bin/python" "$REPO_ROOT/poc_flashvep/deepep_revalidation/vllm_backend_matrix.py" --model-path "$MODEL" --output "$RESULT/trigger-${TRUE_B_MODE:-stage_a}.json" --all2all-backend deepep_high_throughput --request-counts 1 --warmups 0 --iterations 1 --modality vision --image-size 896 --text-target-tokens 790 --max-tokens 1 --kv-cache-memory-bytes 1073741824 --timeout-seconds 14400 2>&1 | tee "$RESULT/run-${TRUE_B_MODE:-stage_a}.log"
