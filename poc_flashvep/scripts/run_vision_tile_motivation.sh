...
result_dir=${1:?usage: run_vision_tile_motivation.sh RESULT_DIR}
report=${2:-${repo_root}/poc_flashvep/reports/flashvep_vision_tile_motivation_report.md}
...
"${deepep_venv}/bin/python" \
  ...
  profile --model-path "${model}" --output-dir "${result_dir}" \
...
python3 \
  "${repo_root}/poc_flashvep/vision_tile_motivation/profile_vision_tile_motivation.py" \
  analyze --output-dir "${result_dir}" --report "${report}" \
