...
export PYTHONPATH="$repo_root/poc_flashvep/live_traffic_matrix_validation/hooks:$repo_root:${PYTHONPATH:-}"
exec "$venv/bin/python" "$repo_root/poc_flashvep/mllm_specific_bottleneck_localization/run_live.py" \
  --model-path "$model" --output-dir "$result" --warmups "${WARMUPS:-1}" --iterations "${ITERATIONS:-2}"
